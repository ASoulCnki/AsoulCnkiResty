...
do
    ...
    if ngx then
        ...
        function _M.tcp(...)
            ...
            if not forced_luasocket_phases[phase] and COSOCKET_PHASES[phase] or forbidden_luasocket_phases[phase] then
...
do
    local function check_phase(phase)
        if type(phase) ~= 'string' then
            ...
            local err = string.format("bad argument #1 to '%s' (%s expected, got %s)", info.name, 'string', type(phase))
            error(err, 3)
        end
    end
